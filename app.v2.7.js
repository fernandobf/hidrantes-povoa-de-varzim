...
console.info('[Hidrantes Póvoa] app v2.7.0');
...
function ensureMapInteractionHandlers() {
  ...
  for (const name of handlers) {
    ...
    if (handler?.enable && handler?.enabled && !handler.enabled()) handler.enable();
  }
}

function restoreLeafletHitTesting(tileLayer) {
  if (!state.map || !els.map) return;

  // Corrige explicitamente o erro das versões anteriores: mapPane/tilePane
  // precisam participar do hit-testing. Os eventos que começam num tile
  // devem propagar até ao contentor do mapa, onde o Leaflet escuta o drag.
  els.map.style.setProperty('pointer-events', 'auto', 'important');

  for (const name of ['mapPane', 'tilePane', 'markerPane', 'popupPane']) {
    const pane = state.map.getPane(name);
    if (pane) pane.style.setProperty('pointer-events', 'auto', 'important');
  }

  // O halo é puramente visual e não deve roubar o rato/toque.
  const highlightPane = state.map.getPane('highlightPane');
  if (highlightPane) highlightPane.style.setProperty('pointer-events', 'none', 'important');

  const normalizeTile = tile => {
    if (!(tile instanceof HTMLElement)) return;
    tile.draggable = false;
    tile.setAttribute('draggable', 'false');
    tile.style.setProperty('pointer-events', 'auto', 'important');
    tile.style.userSelect = 'none';
    tile.style.webkitUserDrag = 'none';
  };

  tileLayer?.on?.('tileload', event => normalizeTile(event.tile));
  els.map.querySelectorAll('.leaflet-tile').forEach(normalizeTile);
...
function initMap() {
  ...
  restoreLeafletHitTesting(tileLayer);
  ...
  state.map.on('layeradd', () => restoreLeafletHitTesting(tileLayer));
...
function updateUserMarker(position) {
  ...
  if (!state.accuracyCircle) {
    state.accuracyCircle = L.circle(latlng, {
      radius: Math.max(position.accuracy || 0, 8), className: 'user-accuracy', weight: 1, interactive: false
    }).addTo(state.map);
  } else {
