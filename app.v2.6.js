...
function markerAssetsForPoint(p) {
  const type = stripText(p?.Tipo).replaceAll(' ', '');
  if (type === 'bocaincendio') {
    return {
      png: './assets/marker_boca_52x80.png',
      svg: './assets/marker_boca.svg',
      css: 'boca-incendio'
    };
  }
  return {
    png: './assets/marker_marco_52x80.png',
    svg: './assets/marker_marco.svg',
    css: 'marco-incendio'
  };
}
...
function createMarkerIcon(p) {
  ...
  const asset = markerAssetsForPoint(p);
  const classes = ['hydrant-marker-img', asset.css];
...
function popupHtml(p) {
  ...
  const asset = markerAssetsForPoint(p);
  return `
    <div class="popup-title"><img src="${asset.svg}" alt=""><strong>${escapeHtml(p.TipoLabel)}${hasValue(p.IDEntidade) ? ` — ${escapeHtml(p.IDEntidade)}` : ''}</strong></div>
...
function ensureMapInteractionHandlers() {
  if (!state.map) return;
  const handlers = ['dragging', 'doubleClickZoom', 'scrollWheelZoom', 'touchZoom', 'boxZoom', 'keyboard'];
  for (const name of handlers) {
    const handler = state.map[name];
    if (handler?.enabled && handler?.enable && !handler.enabled()) handler.enable();
  }
}

function hardenMapInteraction(tileLayer) {
  if (!state.map || !els.map) return;

  // O mapa base e os panes visuais são decoração: nunca devem interceptar
  // o rato/toque. Marcadores/popups reativam pointer-events via CSS.
  const mapPane = state.map.getPane('mapPane');
  if (mapPane) mapPane.style.pointerEvents = 'none';

  const nonInteractivePanes = ['tilePane', 'overlayPane', 'shadowPane', 'tooltipPane', 'highlightPane'];
  for (const name of nonInteractivePanes) {
  ...
  const popupPane = state.map.getPane('popupPane');
  if (popupPane) popupPane.style.pointerEvents = 'none';

  const neutralizeTile = tile => {
  ...
  tileLayer?.eachLayer?.(() => {}); // mantém compatibilidade sem assumir internals.
  tileLayer?.on?.('tileload', event => neutralizeTile(event.tile));
  els.map.querySelectorAll('.leaflet-tile').forEach(neutralizeTile);

  // Última barreira contra o drag nativo de <img> (especialmente Firefox).
  els.map.addEventListener('dragstart', event => {
    if (event.target instanceof HTMLImageElement) event.preventDefault();
  }, true);

  // Se algum plugin/browser alterar handlers durante o ciclo de vida,
  // revalidamos antes de uma interação real.
  const rearm = () => ensureMapInteractionHandlers();
  els.map.addEventListener('pointerenter', rearm, { passive: true });
  els.map.addEventListener('pointerdown', rearm, { passive: true, capture: true });
  els.map.addEventListener('wheel', rearm, { passive: true, capture: true });
  els.map.addEventListener('dblclick', rearm, { passive: true, capture: true });
...
function initMap() {
  ...
  hardenMapInteraction(tileLayer);
  ...
  state.map.on('zoomstart movestart', ensureMapInteractionHandlers);
...
function updateNearest() {
  ...
  if (!state.nearest) {
    if (els.nearestIcon) els.nearestIcon.src = './assets/marker_marco.svg';
    if (!ref) {
    ...
    if (els.nearestIcon) els.nearestIcon.src = markerAssetsForPoint(p).svg;
