...
function initMap() {
  ...
  // Mantemos os handlers nativos do Leaflet. Não bloqueamos mouse, wheel, dblclick
  // nem pointer/touch events manualmente: isto reproduz a interação esperada de
  // um mapa convencional (arrastar, roda, duplo clique e pinça).
  state.map = L.map('map', {
    zoomControl: false,
    minZoom: 10,
    maxZoom: 20,
  ...
  // Revalida o tamanho depois do primeiro layout e garante handlers ativos.
  state.map.whenReady(() => {
